'MAX2871_Command_8.bas for Spectrum Analyzer Freq Synthesis.
...
dim shared as string Sweep, freq, ComStr, portNum, portSpeed, delaystr, FracOpt,Message,LD
dim shared as single Fpfd, Fref, RFOut, FvcoEst, FracErr, Finc, Frfv(401), Initial ', FrfEst
...
dim shared as integer x,y,z,w,h
...
'    portSpeed="9600"
  ...
  print
  print " MAX2871_Command_8.bas by WN2A/MS for PROJECT#3 Signal Generator"
...
Manual:
  ...
  Print " Sorry Dave, I'm afraid I can't do that."	' Warn user: Requested frequency is outside range of MAX2871
  goto Manual
...
Datarow1: 'MAX2871 Initialization
  data  13093080,541097977,1476465218,4160782339,1674572284,2151677957 '374.596154 MHz
Datarow2: 'MAX2871 Initialization, disable LockDetect on Mux Line
  data  13093080,541097977,1073812034,4160782339,1674572284,2151677957 '374.596154 MHz no LD
...
'sub Transmit 'This routine initializes MAX2871 with nSteps=0
  'oldVerbose = Verbose
  'Verbose = 1
  'open "registers.txt" for output as #42 'Douglas Adams was here
  ...
  'for z=0 to 1 'do the Reg 5-4-3-2-1-0 cycle twice
    'for y = 0 to 5 : registerNum = (5 - y)
      ...
      'if Verbose then
        'print #42, hex(Reg(registerNum,0)) 'and here too
...
'  FrfEst = FvcoEst/(Div)
  ...
  for nReg = 0 to 5
    read Reg(nReg, nSteps)  ' Reads from data which is filled by Datarow#
'    print "nReg("; nReg; ") nSteps("; nSteps; ")"
  next
